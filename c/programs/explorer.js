class program {
    ...
    init() {
        ...
        this.wind.setContent('<h1>Explorer</h1><p id="path">c/</p><div id="files"></div>')
    ...
    loadFiles() {
        ...
        for (const [key, value] of Object.entries(fileLookup)) {
            if (key.startsWith(this.pos)) {
                if (!key.replace(this.pos, "").includes("/")) {
                    files.push("<button>" + key.replace(this.pos, "") + "</button>");
        ...
        this.wind.getHtml().querySelector("#files").innerHTML = folders.join("") + "<hr>" + files.join("");
    }
    folderSel(f) {
        this.pos = this.pos + f + "/";
        this.loadFiles();
